...
import * as blog from '@wordpress/blob'
...
export default {
	...
	methods: {
		transformButton() {
			const greeting = (props) => window.wp.element.createElement('span', null, 'Hello ' + props.name + '!')
			wp.element.render(
				window.wp.element.createElement(greeting, { name: 'testing' }),
...
<template>
	<h1>Hello world!</h1>
	<div id="testing"></div>
	<button @click="transformButton">
		Transform Testing Div
	</button>
</template>
